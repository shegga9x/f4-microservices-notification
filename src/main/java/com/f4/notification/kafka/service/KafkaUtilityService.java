package com.f4.notification.kafka.service;

import com.f4.notification.avro.EventEnvelope;
import com.f4.notification.service.dto.NotificationDTO;
import com.f4.notification.kafka.handler.EventDispatcher;
import com.f4.notification.kafka.runner.KafkaJobRunner;
import com.f4.notification.kafka.util.AvroConverter;
...
public class KafkaUtilityService {
    ...
    public String producer_prepareAndAttemptDirectSendReelEvent(
        ...
        try {
            ...
            EventEnvelope envelope = AvroConverter.createNotificationEvent(eventName, null);
    ...
    public void consumeAndProcessMessage(EventEnvelope avroMessage) {
        try {
            retryTemplate.execute(context -> {
                ...
                NotificationDTO serviceNotificationDTO = null;
                ...
                try {
                    ...
                    if (avroMessage.getPayload() != null) {
                        ...
                        serviceNotificationDTO = consumer_mapAvroToServiceNotificationDTO(avroMessage.getPayload());

                        if (serviceNotificationDTO != null) {
                            // Prepare envelope for dispatcher
                            com.f4.notification.kafka.handler.EventEnvelope<JsonNode> envelopeForDispatcher = consumer_prepareEventEnvelopeForDispatcher(
                                    eventName, serviceNotificationDTO);
                            ...
                            CONSUMER_LOG.warn("Service NotificationDTO was null after mapping for event: {}", eventName);
                        }
                    } else {
                    ...
                    // SSE dispatch
                    consumer_dispatchToSseClients(eventName, serviceNotificationDTO, emitters);
    ...
    // existing helper methods: consumer_mapAvroToServiceNotificationDTO,
    ...
    public void consumer_dispatchToSseClients(String eventName, NotificationDTO reelDTO, Map<String, SseEmitter> emittersMap) {
    ...
    // consumer_mapAvroToServiceNotificationDTO now delegates to AvroConverter
    public NotificationDTO consumer_mapAvroToServiceNotificationDTO(com.f4.notification.avro.NotificationDTO avroPayload) {
        CONSUMER_LOG.debug(
                "KafkaUtilityService: Delegating Avro to Service DTO conversion to AvroConverter for payload: {}",
                avroPayload);
        try {
            return AvroConverter.convertToServiceNotificationDTO(avroPayload);
        } catch (Exception e) {
            CONSUMER_LOG.error(
    ...
    public com.f4.notification.kafka.handler.EventEnvelope<JsonNode> consumer_prepareEventEnvelopeForDispatcher(
            String eventName, NotificationDTO serviceDto) {
        if (serviceDto == null) {
            ...
            return new com.f4.notification.kafka.handler.EventEnvelope<>(eventName, null);
        }
        try {
            ...
            return new com.f4.notification.kafka.handler.EventEnvelope<>(eventName, reelJsonNode);
