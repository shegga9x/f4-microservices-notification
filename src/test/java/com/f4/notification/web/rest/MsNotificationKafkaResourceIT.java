...
import com.f4.notification.IntegrationTest;
...
class MsNotificationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ms-notification-kafka/publish?message=value-produce").with(csrf())).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ms-notification-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/ms-notification-kafka/unregister"));
                return;
            }
        }
